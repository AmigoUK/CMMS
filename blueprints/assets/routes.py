...
@assets_bp.route("/expiry-report/email", methods=["POST"])
@supervisor_required
def email_expiry_report():
    """Email the expiry report as PDF to selected contacts."""
    from datetime import datetime
    from models import Contact
    ...
    subject = request.form.get("subject", "Expiry Report").strip()
    ...
    body = f"<p>Please find attached the document expiry report for {g.current_site.name}.</p>"
    body += f"<p>Generated by {current_user.display_name} on {now.strftime('%d %b %Y %H:%M')}.</p>"

    filename = f"Expiry_Report_{g.current_site.code}_{now.strftime('%Y%m%d')}.pdf"
